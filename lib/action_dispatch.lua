...
-- the candidate; with neither key nor fallback_key, nothing is shown -- reserved
-- for edge cases judged impossible to reach in practice, where fallback_key is for
-- ones that are merely rare (e.g. the candidate having stopped resolving to
-- anything between search and execute).
  ...
  if payload == nil then
    ...
    if shown_key ~= nil then
      player.create_local_flying_text({
        text = { shown_key, candidate.label },
