...
local function select_match(best, score, field, positions)
  if score == nil then
    return best
  end
  ...
  if best == nil or score > best.score or (score == best.score and field == "localized_name") then
    return { score = score, field = field, positions = positions }
  end
  return best
end
...
local function build_candidates(
  ...
  for _, prototype in ipairs(prototype_list) do
    if include_hidden or not prototype.hidden then
      ...
      best = select_match(best, internal_score, "internal_name", internal_positions)
      ...
      if type(translated) == "string" then
        ...
        best = select_match(best, display_score, "localized_name", display_positions)
      end
      if best ~= nil then
