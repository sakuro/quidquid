...
describe("SurfaceLogic", function()
  local function planet(overrides)
    local value = {kind = "planet", index = 1, name = "nauvis", label = {"space-location-name.nauvis"},
      icon = "space-location/nauvis", unlocked = true, hidden = false, search_name = "ナウヴィス"}
    for key, entry in pairs(overrides or {}) do value[key] = entry end
    return value
  end

  local function platform(overrides)
    local value = {kind = "platform", index = 2, name = "platform-1", label = "Cargo Express",
      search_name = "Cargo Express", icon = "surface/space-platform", own = true,
      friendly = false, hidden = false, force_name = "Blue Team"}
    for key, entry in pairs(overrides or {}) do value[key] = entry end
    return value
  end
  ...
  it("applies the hidden preference only to accessible surfaces", function()
    for _, value in ipairs({planet{hidden = true}, platform{hidden = true},
      platform{own = false, friendly = true, hidden = true}}) do
